            ...
            <ListGroup>
                <p>Velg hvilken mappe du vil gjenopprette til.</p>
                {
                    this.props.folders.map(folder => {
                          return <ListGroupItem key={folder.id} onClick={this.handleRecovery.bind(this, folder)} color="red" tag="button" action>{folder.fullpath}</ListGroupItem>
                    })
                }
            </ListGroup>
          </ModalBody>
...
const mapDispatchToProps = dispatch => {
  return {
    fetchFilesData: (client_id, selected_folder) => {
      dispatch(fetchFilesData(client_id, selected_folder, true)); //True for is recyclebin
    },
